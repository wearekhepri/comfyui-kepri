...
3. **Background Compositing**: Merges the foreground object onto a chosen background:
   ...
   - **Solid Color**: Fills the background with a specific HEX color.
...
| `background_color` | STRING | `#FFFFFF` | Hex colour used when `background_mode == color`. |
